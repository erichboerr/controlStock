...
public class EntregarImpresoraNuevaServicio extends JDialog implements ActionListener {
	...
	public EntregarImpresoraNuevaServicio() {
		...
		cmbMarca.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent ev) {
				if(cmbServicio.getSelectedIndex()>0) {
					//cmb.cargarCmbMarcaParaAsignarNueva(cmbMarca);
					cmbMarca.insertItemAt("Seleccione una Marca", 0);
					cmbMarca.setSelectedIndex(0);
					cmbModelo.removeAllItems();
					cmbModelo.insertItemAt("Seleccione un Modelo", 0);
		...
		cmbModelo.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent ev) {
				//cmb.cargarCmbModeloParaAsignarNueva(cmbMarca, cmbModelo);				
				cmbModelo.insertItemAt("Seleccione un Modelo", 0);  
				cmbModelo.setSelectedIndex(0);
			}
		});
		...
		btnAceptar.setIcon(
				new ImageIcon(EntregarImpresoraNuevaServicio.class.getResource("/com/controlStock/controlStock/icons/aceptar-48px.png")));
		...
		btnCancelar.setIcon(new ImageIcon(EntregarImpresoraNuevaServicio.class.getResource("/com/controlStock/controlStock/icons/cerrar-ventana-48px.png")));
					...
					if (cmbServicio.getSelectedIndex() > 0) {
						if (cmbMarca.getSelectedIndex() > 0) {
							if (cmbModelo.getSelectedIndex() > 0) {
								if(!txtSerieImpresora.getText().equals("")) {
									String serie= txtSerieImpresora.getText().toUpperCase();
									if (miCoordinador.asignarImpresoraNuevaAServicio(cmbServicio, cmbMarca, cmbModelo, serie)) {
										dispose();
									...
									JOptionPane.showMessageDialog(null, "Debe ingresar un numero de serie ",
											"Advertencia", JOptionPane.ERROR_MESSAGE);
								}							
